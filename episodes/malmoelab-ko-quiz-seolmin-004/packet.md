...
- 기준 레퍼런스: `malmoelab-ko-quiz-seolmin-003` / `daehan-pilot-codex-003`
- 목표 길이 / 비율: `30s` / `16:9`
- 생성일: `2026-08-04T05:34:10Z`
- source issue: [SHO-127](/SHO/issues/SHO-127)
- brief issue: [SHO-128](/SHO/issues/SHO-128)
- 상위 이슈: [SHO-126](/SHO/issues/SHO-126)
- protagonist: `seolmin`
...
- 지금 클릭해야 하는 이유: `30초 안에 설민과 함께 학교 주제의 한국어 문장을 듣고, 따라 읽고, 빈칸으로 다시 확인할 수 있다.`
- 핵심 약속: 영어권 초급 학습자가 `학교`를 단어 카드가 아니라 `학교 앞에서 친구를 만납니다.`라는 생활 문장 맥락으로 익힌다.
- lesson angle: 학교 앞과 친구라는 쉬운 문맥을 먼저 잡고, full sentence 반복 뒤 blank sentence로 기억을 확인한다.
...
- gloss: `school`
- example sentence: `학교 앞에서 친구를 만납니다.`
- blanked sentence: `___ 앞에서 친구를 만납니다.`
- sentence translation: `I meet a friend in front of the school.`
- sentence romanization lock:
  - blanked: `___ apeseo chingureul mannapnida.`
  - full: `hakgyo apeseo chingureul mannapnida.`
- reuse status: `unused_example_selected`
- source base URL: `https://malmoelab.com`
- source ids:
  - word_id: `e00c2be8-8e1a-408f-a2ee-23f17ad8f5cd`
  - sense_id: `14b0dda3-e404-4d3c-854f-736855ee028b`
  - example_id: `6abefc16-6ac6-4953-b0cb-0768912e4634`
...
- `6.0s - 12.0s`: 오늘의 full sentence `학교 앞에서 친구를 만납니다.`를 소개한다.
- `12.0s - 18.0s`: 따라하기 cue와 full sentence read로 발화 리듬을 만든다.
- `18.0s - 24.0s`: blank sentence `___ 앞에서 친구를 만납니다.`로 전환해 recall beat를 만든다.
...
- scene intent:
  ...
  - `scene-2-lesson-intro`: `학교 앞에서 친구를 만납니다.` full sentence를 소개한다.
  - `scene-3-repeat-listen`: `따라해 볼까요?` 뒤에 full sentence를 읽는다.
  - `scene-4-quiz-point`: `___ 앞에서 친구를 만납니다.` blank quiz로 학습 내용을 다시 확인한다.
...
- assumption: `hakgyo` and sentence romanization are render helpers from `source-packet.json`, not source-authored canonical readings.
- block if:
