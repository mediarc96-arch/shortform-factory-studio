...
- [docs/SFS_CONSOLE_USER_MANUAL.md](./docs/SFS_CONSOLE_USER_MANUAL.md)
...
The public domain is served through nginx, while operator access is enforced by
the web app login and session cookie. Set `SFS_AUTH_SECRET`,
`SFS_OPERATOR_USERNAME`, and `SFS_OPERATOR_PASSWORD` in the local `.env` before
